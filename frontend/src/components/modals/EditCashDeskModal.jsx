// src/components/modals/EditCashDeskModal.jsx - ИСПРАВЛЕННАЯ ВЕРСИЯ
import { useState, useEffect } from 'react';
import { X, Wallet, Loader2, DollarSign } from 'lucide-react';

const EditCashDeskModal = ({ isOpen, onClose, onSuccess, onError, cashDesk }) => {
  ...
  // Заполняем форму данными кассы при открытии
  useEffect(() => {
    if (isOpen && cashDesk) {
      setFormData({
        name: cashDesk.name || '',
        current_balance: cashDesk.current_balance?.toString() || '0',
        description: cashDesk.description || '',
        is_active: cashDesk.is_active ?? true
      });
      setErrors({});
    }
  }, [isOpen, cashDesk]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (isNaN(formData.current_balance) || Number(formData.current_balance) < 0) {
      newErrors.current_balance = 'Баланс должен быть положительным числом';
    }
    ...
    try {
      ...
      // НЕ делаем await apiService.put(`/cash-desks/${cashDesk.id}`, cashDeskData);
      ...
      if (error.message?.includes('уже существует')) {
        ...
        onError(error.message || 'Ошибка при обновлении кассы');
      }
    } finally {
      setLoading(false);
    }
  ...
  if (!isOpen || !cashDesk) return null;
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md mx-4">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center space-x-3">
            ...
            <h3 className="text-xl font-semibold text-gray-900">Редактировать кассу</h3>
          </div>
          <button
            onClick={handleClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          {cashDesk && (
            <div className="bg-gray-50 p-3 rounded-lg text-xs text-gray-600 space-y-1">
              <div>Создана: {new Date(cashDesk.created_at).toLocaleDateString('ru-RU')}</div>
              <div>Операций: {cashDesk.transactions_count || 0}</div>
              {cashDesk.calculated_balance !== undefined && (
                <div>Расчетный баланс: {Number(cashDesk.calculated_balance).toLocaleString('ru-RU')} ₽</div>
              )}
            </div>
          )}
...
export default EditCashDeskModal;